...
其中，```js function myfunction(m1,m2){} ```，scrape()方法```+(Method)```為爬取關注的<font color=#FF0000>紅色</font> **今天天氣真好**，行情資料，而save()方法(Method)提供存入MySQL資料庫的功能，可以搭配[Python爬蟲教學]輕鬆學會Python網頁爬蟲與MySQL資料庫的整合方式部落格文章來進行學習。
